...
* **用戶註冊**
  ...
  3) 用戶點擊郵件中的連結完成驗證。
  4) 系統顯示「註冊成功」並導引至登入頁面。

  #### 擴展/替代流程 (Extend)：
  * 在第二步驟，用戶輸入完高科大電子郵件後，系統執行「電子郵件驗證」流程，向郵箱發送驗證連結。
...
* **多維度篩選**
  ...
  #### 基本流程：
  1) 買家開啟篩選面板，設定如科系、價格區間等條件。
  2) 系統根據篩選和排序條件重新查詢資料庫。
  3) 系統執行「瀏覽書籍清單」用例，顯示結果。

  #### 擴展/替代流程 (Extend)：
  * 買家可選擇「科系篩選」、「時間排序」、「價格排序」等功能。
